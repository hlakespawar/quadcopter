...
class Agent():
    ...
    def step(self, action, reward, next_state, done):
        self.buff.add(self.last_state, action, reward, next_state, done)
        
        if len(self.buff) > self.batch_size:
            experiences = self.buff.sample()
    ...
    def act(self, state):
        ...
        #print(noise * 900)
        actions = [min(self.action_high, max(self.action_low, action[i] + self.action_range * noise[i])) for i in range(len(action))]
        #print("{:4.2f} {:4.2f} {:4.2f} {:4.2f}".format(*actions))
        return list(actions)
    ...
    def soft_update(self, local_model, target_model):
        ...
        assert len(local_weights) == len(target_weights), "Local and target not same length"
